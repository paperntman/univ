...
export interface UserNaesinSubject { // 사용자 내신 과목 정보 (UI 및 내부 상태용)
    ...
    curriculumAreaCode?: string | null; // 교과 영역 코드 (API에서 사용)
    curriculumAreaName?: string; // 교과 영역명
...
export interface FilteredUniversity extends UniversityBase { // 필터링된 대학 정보
...
export interface ApiSubjectInfo { // 교과, 과목, 수능 선택과목 등에 공용으로 사용
    ...
    parentCode?: string; // 상위 코드 (예: 과목의 경우 교과 코드)
}
